/**
  ...
  * @brief   This file contains the routinue needed to configure OV9712 
  *          Camera modules.
  ******************************************************************************
  * @attention
  *
...
/**
  * @brief  Configures OV9712 Camera module mounted on STM324xG-EVAL board.
  * @param  ImageBuffer: Pointer to the camera configuration structure
  * @retval None
  */
void Camera_Config(void)
{
  if(Camera == OV9712_CAMERA)
  {
    switch (ImageFormat)
    {
      case BMP_QQVGA:
      {
        /* Configure the OV9712 camera and set the QQVGA mode */
        OV9712_HW_Init();
        OV9712_Init(BMP_QQVGA);
        OV9712_QQVGAConfig();
        break;
      }
      case BMP_QVGA:
      {
        /* Configure the OV9712 camera and set set the QVGA mode */
        OV9712_HW_Init();
        OV9712_Init(BMP_QVGA);
        OV9712_QVGAConfig();
        break;
      }
      default:
